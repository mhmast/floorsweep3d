    ...
    public class KtxFile
    {
        ...
        public static KtxFile Load(Stream s, bool readKeyValuePairs)
        {
            using (var br = new BinaryReader(s))
            {
                ...
                var numberOfMipmapLevels = Math.Max(1, header.NumberOfMipmapLevels);
                var numberOfArrayElements = Math.Max(1, header.NumberOfArrayElements);
                var numberOfFaces = Math.Max(1, header.NumberOfFaces);

                var baseWidth = Math.Max(1, header.PixelWidth);
                var baseHeight = Math.Max(1, header.PixelHeight);
                var baseDepth = Math.Max(1, header.PixelDepth);

                var images = new KtxMipmapLevel[numberOfMipmapLevels];
                for (var mip = 0; mip < numberOfMipmapLevels; mip++)
                {
                    var mipWidth = Math.Max(1, baseWidth / (uint)(Math.Pow(2, mip)));
                    var mipHeight = Math.Max(1, baseHeight / (uint)(Math.Pow(2, mip)));
                    var mipDepth = Math.Max(1, baseDepth / (uint)(Math.Pow(2, mip)));
            ...
            PixelFormat format)
        {
            ...
            var arrayLayers = Math.Max(1, ktxTex2D.Header.NumberOfArrayElements);
            var mipLevels = Math.Max(1, ktxTex2D.Header.NumberOfMipmapLevels);
